...
def test_obv_signal_equals_state_transitions(synthetic_prices_volume):
    ...
    # Fase 1: janela 20.
    out = obv.add_columns(synthetic_prices_volume.copy(), window=20)
    # Fase 2: invariante evento == transições COM referência (a média do OBV) válida
    # ontem — o fim do warm-up não conta como transição real (Task 16).
    state = out["obv_w20_state"]
    sig = out[obv.signal_col(20)]
    ref = out[obv.value_col(20)]
...
def test_obv_no_phantom_onset_at_warmup():
    """
    Por quê: no 1º dia em que a média do OBV fica calculável (fim do warm-up de
    NaN), se o OBV já está acima dela, o onset antigo disparava — o "abaixo de
    ontem" usado na comparação era um NaN coerido para False, não uma observação
    real. Este teste prova que uma série que já nasce "acima" desde o 1º dia válido
    NÃO gera onset nem persistência fantasma.

    Lógica: Entrada (Close sobe todo dia, volume constante → OBV cresce linear e
    supera a própria média) → Fase 1 add_columns(window=3, persist=2) → Fase 2
    confirma que o cenário é real (estado já True no 1º dia válido, idx2) → Fase 3
    nem o onset nem a persistência disparam (nenhuma transição genuína) → Saída.
    """
    # Entrada: Close sobe 1 ponto por dia, volume constante → OBV = [0,100,...,500].
    ...
    out = obv.add_columns(df.copy(), window=3, persist=2)
    # Fase 2: 1º dia válido da média do OBV é idx2 (min_periods=3); estado já True
    # ali (OBV=200 > média=100) — confirma que o cenário é real.
    state = out["obv_w3_state"]
    ...
    assert int(out[obv.signal_col(3)].sum()) == 0
    # Saída: a persistência (ancorada num onset genuíno) também fica silenciosa.
    assert int(out[obv.signal_col(3, persist=2)].sum()) == 0
